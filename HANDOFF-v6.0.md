...
| 标准报价 | 成本 = 纸张原价 + 工艺 + 吊绳 + 邮费（不计算纸张折扣） | CUSTOMER_LEVELS（毛利系数） |

### v6.4 起：纸张不再乘 discount（无重复打折）

v6.4 之前：纸张价格 = 原价 × paper.discount，再 × 客户等级系数 = 双重折扣。
v6.4 起：纸张价格 = 原价，× 客户等级系数（或直接系数）= 单次折扣。
