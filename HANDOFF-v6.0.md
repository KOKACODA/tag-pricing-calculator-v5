...
├── v6.4-CHANGELOG.md   # v6.4 总结文档
├── v6.5-CHANGELOG.md   # v6.5 总结文档
└── v6.6-CHANGELOG.md   # v6.6 总结文档
```
...
| 模式 | 计算方式 | 系数来源 |
|---|---|---|
| 直接系数计算（默认） | 成本 = 纸张原价 + 工艺（不计算吊绳/邮费/纸张折扣） | 每 Sheet 专属 `directCoeff`（读取报价表表格三行：直接系数档位/最高倍数/最低倍数），无直接系数的 Sheet 按客户等级系数计算 |
| 标准报价 | 成本 = 纸张折后价 + 工艺 + 吊绳 + 邮费 | CUSTOMER_LEVELS（毛利系数） |

### v6.5 起：标准报价打折 / 直接系数不打折

v6.5 起：折扣只在标准报价触发。
- 标准报价模式：纸张价格 = 原价 × `paper.discount`（打折），再 × 客户等级系数 = 最终报价
- 直接系数模式：纸张价格 = 原价（不打折），× 直接系数（或客户等级系数回退）= 最终报价
- `paper.discount` 字段：标准报价参与计算，直接系数模式不参与

### v6.6 起：导入导出健壮性修复

- **工艺跨报价表隔离**：导入纸张始终生成唯一 ID（`paper_import_N`），不再复用默认 `paper id`，各报价表工艺完全隔离；Excel 无工艺区时从默认配置复制同名简称纸张的工艺到新 ID 下
- **档位表头空列检测**：表头中间空列导入时提示「价格可能错位」，避免价格静默错位
- **折扣系数上限校验**：折扣系数校验范围 0~10，超出回退为 1 并提示
- **全量导入数据校验**：`importFullData` 导入前执行 `validateImportedData`
...
报价表新增「批量直接报价」数据板块，位于直接系数档位表下方，格式三行（与报价表表格格式一致）：

```javascript
// data.js 中每张纸的 batchDirect 字段
{
  "batchDirect": {
    "maxArea": 7004,   // 最大面积（出血后）
    "prices": { "1000": 168, "2000": 330, "3000": 500, "4000": 650, "5000": 780, "10000": 1480, "20000": 2900 }
  }
}
```

- `maxArea` = 最大面积（出血后），面积 ≤ maxArea 时按批量直接报价
- `prices` = 批量档位 → 固定价格（不打折、不乘直接系数、不乘面积系数）
- 工艺费用直接叠加（批量纸张价 + 批量纸张工艺费）
- 档位无对应价格时显示"无该批量报价"占位，读取但不计算
- 目前仅 40C 棉麻布配置（maxArea 7004 mm²，对应 65×100mm 无出血）
- 无批量直接报价的 Sheet：`batchDirect` 为 `null` 或缺失，按常规报价计算
- Excel 模板/导出/导入均支持三行格式；无批量直接报价的 Sheet 提供占位行（最大面积与价格留空）
...
| `paper.batchDirect` | 字段 | 批量直接报价（v7.0）：`{ maxArea, prices }` | 随 `paperConfig` |
...
输入：纸张选择 + 尺寸 + 工艺 + 批量档位
  ↓
1. 每张纸：面积 → 匹配规格代码 → 查价格表 → 纸张单价
2. 批量直接报价判断（v7.0）：面积（出血后）≤ batchDirect.maxArea → 直接按批量价格（不打折、不乘系数）
3. 纸张总价 = Σ(单价 × 面积系数) × 折扣（批量直接报价纸张不计入此项）
4. 工艺费用 = Σ(每张工艺价格)（批量直接报价纸张的工艺费直接叠加）
5. 成本 = 纸张 + 工艺 + 批量直接报价纸张价 + 批量直接报价工艺费（不计算吊绳和邮费）
6. 直接系数 = getDirectCoeffsForTier(档位) → 按档位规则调整（批量直接报价纸张不参与系数计算）
7. 报价 = 成本 × 直接系数（3个等级，右上角显示系数徽章）
```
