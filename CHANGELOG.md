# 更新日志

所有 notable 变更均记录在此文件中。  
格式参考 [Keep a Changelog](https://keepachangelog.com/zh-CN/)。

---

## [v7.10] - 2026-09-01

### 新增
- **运行日志机制**：自动在 localStorage 记录关键操作（计算、模式切换、报价表切换、导入导出、设置保存、快照、重置）与全局错误（`window.onerror` / `unhandledrejection`）
  - 计算日志带 3 秒节流，避免输入过程刷屏；日志最多保留最近 200 条，防止存储无限增长
  - 个人主页新增「运行日志」卡片，支持刷新查看、导出 JSON、一键清除
  - 日志仅存本机浏览器，不对外发送
- **版本管理补齐**：为 v7.0~v7.9 补打并推送 Git 标签（`v7.0-stable` ~ `v7.9-stable`），与既有标签格式统一

### 修改
- 所有文件版本号统一更新至 v7.10

---

## [v7.9] - 2026-09-01

### 新增
- **成本合计高亮放大**：计算器页「成本合计」框体和字体放大，并以红色高亮显示（红色边框 + 浅红背景 + 红色大号金额），醒目突出最终成本
- **默认报价卡片显示开关**：个人主页「报价设置」新增「报价显示」区块，可控制计算器页「默认报价（原邮费）/直接系数报价」客户等级报价卡片的显示/隐藏
  - 默认隐藏该部分（计算器页更简洁），可在报价设置勾选开启
  - 关闭/隐藏时「临时毛利系数」「邮费快速修改」等仍正常显示
  - 该设置保存在个人配置中（`APP_PROFILE.showDefaultPriceCards`），保存后立即生效

### 修改
- 计算器页「默认报价（原邮费）」报价卡片区域包装为 `#defaultPriceSection`，默认隐藏
- 所有文件版本号统一更新至 v7.9

---

## [v7.8] - 2026-09-01

### 新增
- **2号报价表默认数据更新**：新增「702铜版纸」（2号报价表-3：702克 A级铜版纸 双面过哑胶，厚度 0.85mm）
  - 10 个规格（002-100），直接系数档位 1000-50000，工艺 5 组（无色压凹、套字压凹、烫金 小面积单/双面、击凸）
  - 3000/4000 档位报价表内无价格，查询页显示「无该批量定价」占位，不参与计算
- **v7.8 数据迁移**：旧版 localStorage 用户首次加载时自动补充 702铜版纸 及其工艺，并重排至 2号报价表第 3 位（400铜版纸 之后）

### 修改
- 2号报价表由 15 张纸扩展为 16 张纸，新增纸张 id 为 `paper2_16`
- 所有文件版本号统一更新至 v7.8

---

## [v7.7] - 2026-08-28

### 新增
- **个人主页仓库链接框体**：在「报价设置」卡片上方新增独立的「仓库链接」卡片
  - 以超链接形式展示（不显示原始 URL），链接指向 `https://box-wms.pages.dev/`
  - 新窗口打开（`target="_blank"` + `rel="noopener noreferrer"`），新增 `warehouse-link` 样式，悬停高亮

### 修改
- **面积系数规则说明更新**：明确「报价表内有覆盖当前面积的规格时（含超过 10000mm² 的规格）优先按报价表价格计算，仅当面积超过报价表所有规格的最大面积时才使用面积系数」
- 所有文件版本号统一更新至 v7.7

### 修复
- **JS 脚本缓存参数同步**：`index.html` 中 `js/data.js?v=7.4` / `js/app.js?v=7.4` 更新为 `?v=7.7`，避免浏览器按旧 URL 缓存导致用户继续加载 v7.4 旧版 JS（2/3/4 号报价表、快捷切换等不生效）

---

## [v7.6] - 2026-08-28

### 新增
- **计算器页快捷切换报价表**：在"参数输入"标题行右侧新增"报价表"快捷下拉框（`quickPriceListSelector`），无需进入报价表查询页即可切换当前报价表
  - 与报价表查询页的 `priceListSelector` 双向联动，切换后纸张材质/下拉列表自动更新
  - 新增 `title-controls` / `quick-price-switcher` 样式，移动端自适应

### 修改
- `renderPriceListSelector` 重构为同时渲染两个下拉框并同步选中态
- `onPriceListChange` 兼容两个下拉框触发来源，切换后同步选中态
- 所有文件版本号统一更新至 v7.6

---

## [v7.5] - 2026-08-27

### 新增
- **默认数据报价表组**：新增 2号、3号、4号报价表，作为默认数据组接入系统
  - 2号报价表：15 张纸张，3号报价表：12 张纸张，4号报价表：6 张纸张（共 33 张）
  - 26 张纸张关联工艺数据（无色压凹、烫金等）
  - 新增 `GROUP_NAME_MAP` 小组名映射，报价表导出/展示按小组名区分
- **v7.5 数据迁移**：旧版 localStorage 用户首次加载时自动补充 2/3/4 号报价表、纸张与工艺默认数据，无需手动重置

### 修改
- 报价表组配置 `DEFAULT_PRICE_LISTS` 扩展为 4 个默认报价表（1/2/3/4 号），吊绳/邮费仍全局共享
- 所有文件版本号统一更新至 v7.5

---

## [v7.4] - 2026-08-27

### 安全加固
- **P0 高危修复：移除 Git 仓库远程 URL 中明文嵌入的 GitHub Token**，改为从环境变量 `GH_TOKEN` 通过凭据助手注入，不再落盘明文凭据
- **CSP 收紧**：移除 `script-src` 中的 `'unsafe-inline'`，将 10 处功能占位卡片的 `onclick` 内联事件改为 `data-toast` + `addEventListener` 绑定
- **SheetJS 增加 SRI 完整性校验**（`sha384` + `crossorigin="anonymous"`），防止 CDN 投毒；移除 HTML 中冗余的 `<script defer>` 加载（懒加载已覆盖）
- **新增 `_headers` 安全响应头**：`X-Frame-Options: DENY`、`X-Content-Type-Options: nosniff`、`Referrer-Policy`、`Permissions-Policy` 等
- **`.wrangler/` 加入 `.gitignore`**，并移除已提交的 `.wrangler/cache/pages.json`（含 account_id 的构建缓存不再入库）
- 所有文件版本号统一更新至 v7.4

---

## [v7.3] - 2026-08-18

### 修改
- **强制面积规格映射（不读取表格数据）**：面积（加出血后）范围强制对应规格代码
  - 面积 4000-4999 → 规格 005
  - 面积 5000-5499 → 规格 055
  - 面积 5500-5999 → 规格 006
  - 该映射在 `matchSpec` 中强制生效，覆盖表格 maxArea 数据；未命中区间仍按原逻辑匹配
- 所有文件版本号统一更新至 v7.3

---

## [v7.2] - 2026-08-17

### 修改
- **个人主页 UI 精简优化**：合并冗余框体，减少视觉分割
  - 卡片数量由 9 个精简为 5 个：报价设置、计算规则说明、数据管理、更多功能、全局恢复
  - 「客户建议价格·毛利系数」「金额显示」「公司信息与报价单」「默认选项」合并为「报价设置」卡片，内部以分区标题（section-title）分隔
  - 「直接系数说明」「面积系数说明」合并为「计算规则说明」卡片，改为可折叠的规则块（details/summary），默认展开直接系数
  - 「关于与版本」信息并入「更多功能」卡片底部
  - 头部用户信息改为无框展示（profile-hero），不再使用独立卡片
  - 分区标题采用小号大写样式，行间距收紧，视觉层次更清晰
- **直接系数计算顺序调整**：工艺价先加到该纸张价上，再乘直接系数
  - 公式由「纸张价 × 系数 + 工艺价」改为「(纸张价 + 工艺价) × 系数」
  - 每张纸独立累计自己的工艺费（sheetCraftTotal），报价明细卡片显示 `(纸张价 + 工艺价) × 系数 = 报价`
  - 临时系数报价结果同步改为 `(纸张价 + 工艺价) × 系数` 格式
  - 无直接系数的纸张系数为 1，等价于「纸张价 + 工艺价」，行为不变
- 所有文件版本号统一更新至 v7.2

---

## [v7.1] - 2026-08-17

### 新增
- **数据库更新为默认**：导入新报价表 `KOKALabel1号报价表_20260817_1714.xlsx` 作为默认数据库
  - 10 张纸完整配置更新（350铜版纸、400铜版纸、702铜版纸、606米白卡、500白卡纸、海成专用、700布纹纸、600牛皮纸、40C棉麻布、602米白卡）
  - 更新 400铜版纸、702铜版纸、500白卡纸、602米白卡 的规格价格与工艺价格
  - 40C 棉麻布批量直接报价价格沿用新表（1000 张 168 元 / 2000 张 330 元 / 3000 张 500 元 / 4000 张 650 元 / 5000 张 780 元 / 1 万张 1480 元 / 2 万张 2900 元），最大面积（出血后）沿用 7004 mm²
  - 无直接系数纸张（700布纹纸、40C棉麻布）保留空直接系数模板
- 所有文件版本号统一更新至 v7.1

---

## [v7.0] - 2026-08-17

### 新增
- **批量直接报价板块（新计算板块）**：报价表新增「批量直接报价」数据板块，位于直接系数档位表下方
  - 板块三行格式（与报价表表格格式一致）：`批量直接报价最大面积` / `批量直接报价档位` / `批量直接报价价格`
  - 有批量直接报价的 Sheet 填写实际最大面积/档位/价格；无批量直接报价的 Sheet 提供占位行（最大面积与价格留空），方便填写后重新导入
  - 目前仅 40C 棉麻布配置：最大面积（出血后）7004 mm²（对应 65×100mm 无出血），档位 1000/2000/3000/4000/5000/10000/20000 张，价格 168/330/500/650/780/1480/2900 元
- **批量直接报价计算逻辑**：
  - 面积（出血后）在最大面积范围内时，价格直接按批量报价，不打折、不乘直接系数、不乘面积系数
  - 工艺费用直接叠加（批量纸张价 + 批量纸张工艺费）
  - 选择档位无对应批量价格时（如 500 张），显示"无该批量报价"占位，读取但不计算
  - 示例：60×90mm（无出血，出血后 5859mm² ≤ 7004）选择 2000 张 → 报价 330 元；额外选择红色工艺 → 330 + 30 = 360 元
- **报价表组查询页显示批量直接报价板块**：切换 Sheet 时显示该 Sheet 的批量直接报价表（最大面积 + 批量价格），无配置的 Sheet 显示"该纸张无批量直接报价，按常规报价计算"占位提示
- **首页纸张设置提示**：有批量直接报价的纸张（如 40C 棉麻布）显示绿色提示"批量直接报价：面积 ≤ 最大面积 mm²（出血后）时按固定批量价报价，工艺费用直接叠加"，不再误显示"请切换为标准报价"提示

### 修改
- **Excel 模板/导入/导出支持批量直接报价**：模板下载、单表导出、批量导入均支持读取/写入批量直接报价三行；导入时表格未填有效批量直接报价则继承默认配置（匹配简称），否则为 null
- **报价结果适配**：
  - 批量直接报价纸张不显示折扣计算过程（价格固定，不打折）
  - 工艺费用合计 = 常规工艺费 + 批量直接报价纸张工艺费
  - 直接系数明细卡片显示"批量直接价"标签与固定价格
  - 临时系数功能中批量直接报价纸张禁用系数输入，固定显示批量直接价
- 所有文件版本号统一更新至 v7.0

---

## [v6.14] - 2026-08-17

### 修改
- **直接系数明细卡片换行显示**：报价结果直接系数报价卡片中，每张纸的「名称 / 计算式 / 无直接系数标签」改为纵向换行排列，卡片竖向自动延长，不再横向溢出框体
  - 40C棉麻布：`¥90.00 × 0.75（折扣） = ¥67.50` 单独一行显示
  - 700布纹纸：`¥35.00` 单独一行显示
  - 工艺费用单独一行显示
- **全局 UI 适配检查**：
  - 价格卡片（`.price-card`）增加 `min-width: 0` 与换行保护，防止内容溢出
  - 临时系数输入项增加长名称省略号与最大宽度限制
  - 纸张材质下拉框保持允许溢出（绝对定位 + 独立滚动），不受影响
  - 报价表查询页表格保持水平滚动容器，数据多时横向滚动
- 所有文件版本号统一更新至 v6.14

---

## [v6.13] - 2026-08-17

### 修改
- **无直接系数提示切换修复**：切到标准报价后，"该纸张无直接系数，请切换为标准报价计算"提示自动隐藏（仅在直接系数模式显示）
- **报价结果表格单价显示原价**：各纸张尺寸/面积/代码表格的「单价」列显示 Sheet 原始价格（不打折、不乘系数），直接系数与标准报价模式一致
- **纸张折后价合计显示计算过程**：标准报价模式下，有折扣的纸张显示 `¥原价 × 折扣`（如 `¥ 90 × 0.75`），无折扣纸张直接显示价格，合计不变
- **报价结果溢出修复**：结果表格与合计行支持长内容自动换行，多纸张计算不再溢出框体
- 所有文件版本号统一更新至 v6.13

---

## [v6.12] - 2026-08-17

### 修改
- **无直接系数纸张计算逻辑修复**：直接系数模式下，无直接系数的纸张（如 40C棉麻布、700布纹纸）不乘客户等级系数
  - 直接显示原价 × discount（折扣），结果正确符合需求
  - 40C棉麻布保留折扣（75折）显示，700布纹纸不打折直接显示原价
  - 以后导入其他报价表无直接系数的纸张都遵循此规则
  - 修复徽章统计：只统计有直接系数纸张的系数范围，避免无直接系数纸张污染徽章
  - 优化临时系数结果显示：无直接系数且系数为 1 时不显示冗余的 `× 1`
- 所有文件版本号统一更新至 v6.12

---

## [v6.11] - 2026-08-15

### 新增
- **数据库更新为默认**：导入新报价表 `KOKALabel1号报价表_20260815_1708.xlsx` 作为默认数据库
  - 10 张纸完整配置更新（350铜版纸、400铜版纸、702铜版纸、606米白卡、500白卡纸、海成专用、700布纹纸、600牛皮纸、40C棉麻布、602米白卡）
  - 更新各纸张直接系数档位与工艺价格（共 40 个工艺）
  - 无直接系数纸张（700布纹纸、40C棉麻布）保留空直接系数模板

### 修改
- **055 档位区间固定为 5500-6000**：代码 055 的 maxArea 固定为 6000（不依赖表格），前一档代码 005 的 maxArea 调整为 5500，使 055 精确覆盖 5500-6000 面积区间，6000 以上匹配 006
- 所有文件版本号统一更新至 v6.11

---

## [v6.10] - 2026-08-15

### 修改
- **055 档位 maxArea 调整**：所有 10 张纸的代码 055 最大含出血面积由 5684 改为 5500，面积 5500 以上将匹配下一档代码 006

### 修改
- 所有文件版本号统一更新至 v6.10

---

## [v6.9] - 2026-08-15

### 新增
- **报价结果面积/代码列红色高亮**：报价结果「各纸张尺寸 / 面积 / 代码」表格中，「面积 (mm²)」列与「代码」列字体改为红色加粗，便于快速定位面积与代码档位
- 代码匹配逻辑核查：055 档 maxArea 为 5684（数据来自报价表），面积 5500~5684 匹配 055 为正确行为，下一档 006 从 6264 起

### 修改
- 所有文件版本号统一更新至 v6.9

---

## [v6.8] - 2026-08-15

### 新增
- **直接系数每张纸独立计算**：直接系数模式下，报价结果不再用统一系数，而是每张纸按各自 Sheet 的直接系数单独乘算后累加
  - 有直接系数的纸张：按该纸档位直接系数（最高→普通客户，最低→大客户，中间等级等差插值）
  - 无直接系数的纸张：回退到客户等级系数（普通 1.2 / 优质 1.15 / 大客户 1.1），明细中标注「无直接系数」
  - 工艺费用不乘系数，直接累加，防止多纸张混合时工艺计算错误
- **直接系数明细卡片**：报价结果 3 个等级卡片改为明细卡片，卡片内逐张显示「纸张名 × 系数 = 贡献价」与工艺费用，右上角徽章显示系数范围（如 ×1.45-1.6）
- **临时直接系数每纸独立**：临时直接系数由单一输入框改为随纸张数量动态生成的 N 个输入框（纸张1/纸张2/纸张3…）
  - 有直接系数的纸张默认值 = 该纸当前档位普通客户系数
  - 无直接系数的纸张默认值 = 1，报价时按「有折扣打折扣、无折扣则原价」计入（如 40C棉麻布 ¥90 × 0.75 = ¥67.50）
  - 输入时实时重算并显示「临时报价结果」明细卡片（含工艺）
- 标准报价模式完全保持不变（统一系数卡片、统一临时毛利系数、邮费快速修改均照旧）

### 修改
- 所有文件版本号统一更新至 v6.8

---

## [v6.7] - 2026-08-15

### 新增
- **直接系数档位表独立显示**：报价表组查询页中，直接系数档位从工具栏内联文本改为「吊牌特殊工艺价格表」下方的独立框体表格（`直接系数档位表`）
  - 表格按「最高倍数 / 最低倍数」两行 × 各批量档位列展示，与工艺价格表风格一致
  - 无直接系数的纸张显示提示「该纸张无直接系数（最高/最低倍数为空），按标准报价计算」
  - 移除工具栏中已无用的 `paperDirectCoeff` 内联元素及对应 CSS 样式

### 修改
- 所有文件版本号统一更新至 v6.7

---

## [v6.6] - 2026-08-15

### 修复
- **工艺配置跨报价表覆盖（高严重度）**：导入的 Sheet 若简称与默认纸张相同，会复用默认 `paper id`，导致新报价表的工艺覆盖原报价表工艺（静默丢失）。现改为导入纸张始终生成唯一 ID（`paper_import_N`），各报价表工艺完全隔离；Excel 无工艺区时从默认配置复制同名简称纸张的工艺到新 ID 下，保持"导入后工艺可用"的既有行为
- **档位表头空列价格错位（中严重度）**：修改表格时若档位表头中间留空列（误删列/合并单元格残留），价格会静默错位。现导入时检测表头中间空列并提示「价格可能错位，请检查后重新导入」；末尾空列不误报
- **折扣系数上限校验（中严重度）**：Excel 导入只校验 `discount <= 0`，误填 >10 会被接受导致价格放大。现校验范围 0~10，超出回退为 1 并提示
- **全量导入缺少数据校验（低严重度）**：`importFullData` 未调用 `validateImportedData`，损坏/恶意 JSON 可能导致运行时错误。现与本地备份恢复一致，导入前执行数据校验
- **导出版本号未更新（低严重度）**：`exportFullData` / `exportLocalBackup` 仍写 `version: "6.0"`，现更新为 `6.5`，避免未来按版本迁移时误判

### 修改
- 所有文件版本号统一更新至 v6.6
- 补充 v5.2.1 独立 CHANGELOG 条目（此前内容并入 v5.2「修复」小节，未单独记录）

---

## [v5.2.1] - 2026-08-06

### 修复
- **浏览器缓存问题**：JS/CSS 引用添加版本号 `?v=5.2`，强制浏览器加载最新文件，修复旧版 `data.js` 被缓存导致全局恢复后系数仍为 1.3/1.15/1.05 的问题
- **v5.2 迁移逻辑**：自动检测 localStorage 中的旧默认系数（1.3/1.15/1.05）并迁移为新默认（1.2/1.15/1.1），仅迁移与旧默认完全一致的值，用户自定义系数不受影响

> 注：该版本此前内容并入 v5.2「修复」小节记录，v6.6 起补为独立条目，与 git 提交 `01ffaf2` 及标签 `v5.2.1-stable` 对应。

---

## [v6.5] - 2026-08-15

### 修复
- **标准报价模式恢复纸张折扣**：标准报价模式下纸张价格 = 原价 × `paper.discount`（打折），纸张折后价合计显示折后价
- **直接系数模式不打折**：直接系数模式下纸张始终用原价（不打折），各纸张表格单价显示原价
- 折扣只在标准报价触发，直接系数模式不触发

### 修改
- 所有文件版本号统一更新至 v6.5

---

## [v6.4] - 2026-08-14

### 修复
- **去掉纸张层面折扣（discount 不再参与计算）**：所有模式下纸张均按原价计算，折扣只通过客户等级系数（标准模式）或直接系数（直接系数模式）体现，解决"重复打折"问题
- **各纸张表格单价显示原价**：单价列不再显示折后价，统一显示原价（如 40C棉麻布 1000张显示 ¥90 而非 ¥67.50）
- **查询页直接系数显示统一校验**：改用 `paperHasDirectCoeff` 判断，确保三行完整且数量一致时才显示

### 修改
- 所有文件版本号统一更新至 v6.4

---

## [v6.3] - 2026-08-14

### 新增
- **无直接系数纸张提示**：纸张设置卡片中，无直接系数的 Sheet（如 700布纹纸、40C棉麻布）在纸张材质下拉框下方高亮显示「该纸张无直接系数（最高/最低倍数为空），请切换为标准报价计算」，内联显示不弹窗
- **提示覆盖所有导入场景**：导入其他报价表后，凡是无直接系数、或直接系数最高倍数/最低倍数为空、或档位与系数数量不一致的 Sheet 均显示该提示
- **直接系数有效性校验增强**：`paperHasDirectCoeff` 要求「直接系数档位 / 最高倍数 / 最低倍数」三行均非空且长度一致才算有效，任何不完整配置均视为无直接系数

### 修改
- 所有文件版本号统一更新至 v6.3

---

## [v6.2] - 2026-08-14

### 新增
- **直接系数改为只读报价表表格**：取消个人主页「直接系数设置」卡片（全局设置），直接系数只读取报价表（Excel）每个 Sheet 的「直接系数档位 / 最高倍数 / 最低倍数」三行数据
- **三行格式**：每张纸 `directCoeff` 改为 `{ tiers, max, min }` 数组格式；最高倍数→普通客户，最低倍数→大客户，中间等级等差插值
- **报价表组查询页新增直接系数显示**：工具栏展示当前 Sheet 的档位与最高/最低系数徽章
- **默认报价表更新**：`DEFAULT_PAPER_CONFIG` 全面更新为最新表格数据（350铜版纸示例：500-3000张 最高1.3/最低1.25，5000张起 最高1.2/最低1.15）

### 修改
- **有直接系数的 Sheet 不再计算折扣**：直接系数模式下使用原价（不打折），隐藏「纸张折后价合计」行
- **无直接系数的 Sheet 按标准报价计算**：乘折扣系数，使用客户建议价格 · 毛利系数
- **导出/导入/模板全面支持三行格式**：有直接系数填实际值，无直接系数提供占位档位行（最高/最低留空）
- **保留临时直接系数输入**：报价结果页临时系数快速设置保留，仅影响当前显示
- 所有文件版本号统一更新至 v6.2

---

## [v6.1] - 2026-08-14

### 新增
- **导入新报价表为默认数据库**：`DEFAULT_PAPER_CONFIG` 与 `DEFAULT_CRAFT_CONFIG` 全面更新为 2026-08-14 新版报价表数据（10 张 Sheet，含 602米白卡）
- **直接系数改为每个 Sheet 单独配置**：每张纸（Sheet）拥有独立的 `directCoeff` 字段
  - 702铜版纸 已配置专属档位规则：1000-4000张 最高1.6/最低1.5；5000-10000张 最高1.5/最低1.45；20000张以上 最高1.45/最低1.4
  - 其余 Sheet 提供 `directCoeff` 占位模板（空 `tierRules`），方便后续修改
- **Excel 模板新增「直接系数档位规则」行**：模板改为基于默认数据动态生成（10 张 Sheet，含 602米白卡），所有 Sheet 均含该行，格式 `最低张数-最高张数:最高系数/最低系数`，多条用 `;` 分隔；702铜版纸 已填好示例值
- **Excel 导入支持读取直接系数**：重新导入报价表时，若 Sheet 填写了「直接系数档位规则」则自动解析；未填写时匹配默认简称则继承默认配置，否则提供占位
- **Excel 导出包含直接系数**：导出时自动写入「直接系数档位规则」行，保证导入导出可往返

### 修改
- **直接系数模式不再触发纸张折后价**：直接系数模式下使用原价（不打折），报价结果隐藏「纸张折后价合计」行
- **首页默认打开直接系数计算**：`calcMode` 默认值改为 `direct`，启动时同步隐藏吊绳/邮费输入区域
- **模式切换 UI 调换**：切换按钮顺序改为「直接系数计算」在前、「标准报价」在后
- 所有文件版本号统一更新至 v6.1

---

## [v6.0] - 2026-08-13

### 新增
- **直接系数档位规则可编辑**：`DIRECT_COEFF_TIER_RULES` 从硬编码常量改为可持久化变量
- 个人主页直接系数设置区域新增「批量档位系数规则」编辑器
- 支持新增/删除/修改每条规则的张数范围和最高/最低系数
- 最大张数留空表示无上限
- 修改后实时保存 localStorage 并立即重算

### 修改
- 导出/导入备份包含 `directCoeffLevels` 和 `directCoeffTierRules`
- 「一键恢复全局默认」包含档位规则重置
- 标准报价和直接系数模式统一显示系数徽章（移除 `calcMode === "direct"` 条件）
- 所有文件版本号统一更新至 v6.0

---

## [v5.9] - 2026-08-13

### 新增
- **直接系数按批量档位自动调整**：新增 `DIRECT_COEFF_TIER_RULES` 配置
  - 1000-4000张: 最高1.6/最低1.5
  - 5000-10000张: 最高1.5/最低1.45
  - 20000张以上: 最高1.45/最低1.4
- 新增 `getDirectCoeffsForTier()` 按档位等差插值计算各级系数
- 直接系数模式报价卡片右上角显示系数徽章（×1.6 等）
- 临时系数卡片同样显示系数徽章

### 修改
- `calculate()` 函数直接系数模式改用 `getDirectCoeffsForTier(tier)` 获取系数
- 个人主页直接系数说明区域增加档位规则说明

---

## [v5.8] - 2026-08-12

### 新增
- **报价结果表格代码列手动切换**：上下箭头按钮切换代码规格
- 新增 `getPaperDisplayCodes()` 和 `findSpecByDisplayCode()` 辅助函数
- 新增 `onCodeSwitch()` 函数处理手动切换逻辑
- `sheetsState` 新增 `manualCode` 字段保留手动选择

### 修改
- `calculate()` 支持 `manualCode` 覆盖自动匹配的 spec
- 切换纸张或修改尺寸时自动清除 `manualCode` 恢复自动匹配
- 手动切换后代码显示蓝色 + 「手动」标签
- 007/008 合并处理保持一致

---

## [v5.7] - 2026-08-12

### 新增
- **直接系数计算模式**：参数输入旁新增模式切换器（标准报价 / 直接系数计算）
- 直接系数模式不计算吊绳和邮费，成本 = 纸张 + 工艺
- 个人主页新增独立直接系数等级设置（默认 1.5 / 1.45 / 1.4）
- 新增 `DEFAULT_DIRECT_COEFF_LEVELS` 和 `DIRECT_COEFF_LEVELS` 配置

### 修改
- 直接系数模式下隐藏吊绳类型、收货地区、吊绳费用、邮费行、邮费快速修改栏
- 临时毛利系数标签自动切换为「临时直接系数」
- 默认报价标签切换为「直接系数报价」

---

## [v5.6] - 2026-08-09

### 修复
- **关键Bug：`.card` / `.sheet-card` 的 `overflow: hidden` 截断下拉框**：移除容器的 overflow:hidden，这是纸张材质下拉框显示不全的根本原因
- **`.result-row` 溢出**：移除 overflow:hidden，防止长价格文本（含面积系数公式）被截断

### 响应式适配优化
- **全平台断点体系**：4 级断点覆盖所有设备
  - `≥1024px`（PC/iPad横屏）：增大间距和标题字号
  - `≤768px`（iPad竖屏）：纸张选项保持两列，表格触摸滚动
  - `≤480px`（手机）：导航字号自适应，面积系数行堆叠，价格卡片纵向
  - `≤360px`（超小屏）：表格紧凑显示，导航进一步缩小
- **组件 clamp() 流式缩放**：以下组件的固定 px 全部改为 clamp() 自适应
  - `.price-list-switcher`（报价表切换器）
  - `.area-coeff-row`（面积系数提示行）
  - `.custom-coeff-bar` / `.custom-coeff-input`（临时毛利系数栏）
  - `.shipping-override-bar` / `.shipping-override-input`（邮费快速修改栏）
  - `.price-section-label`（报价区分标签）
- **iOS 安全区域**：保留 `env(safe-area-inset-*)` 适配

---

## [v5.5.2] - 2026-08-09

### 修复
- **纸张下拉框无法滚动到底部**：`adjustDropdownPosition()` 重写为动态计算视口可用空间
  - 根据 trigger 位置实时设置 `max-height`，确保下拉框始终在视口范围内
  - 下方空间不足时自动翻转上方，并根据上方空间设置 max-height
  - 关闭下拉框时清理内联 maxHeight 样式
  - 滚轮边界判断阈值从 1px 增至 2px 防止亚像素计算问题

---

## [v5.5.1] - 2026-08-09

### 修复
- **纸张材质下拉框描述文字被截断**：移除 `.paper-desc` 的 `white-space:nowrap` / `overflow:hidden` / `text-overflow:ellipsis`
  - 改为 `word-break:break-all` + `line-height:1.4`，描述文字可换行完整显示
  - 同步移除响应式区域的截断样式
  - 仅修改纸张材质下拉框，不影响其他下拉框

---

## [v5.5] - 2026-08-09

### 重大新增
- **面积系数计算（超 10000mm²）**：面积超过 10000 时按面积系数计算价格
  - 新增 `calcAreaCoefficient()` 函数：面积 ÷ 10000，四舍五入保留两位小数
  - `matchSpec()` 超过 10000 时使用最大规格（code 100）+ 面积系数，不再报错
  - `calculate()` 中将系数应用到纸张价格（折扣前和折后价均乘以系数）
  - 举例：面积 10289 → 系数 1.03 → 80 × 1.03 = 82.4；面积 34289 → 系数 3.43 → 80 × 3.43 = 274.4

### UI 新增
- **面积系数提示行**：报价结果区新增黄色提示行，超 10000 时显示，平常隐藏
  - 显示当前面积系数值和触发原因说明
- **纸张价格显示格式更新**：触发面积系数时显示「¥ 基础价 × 系数 = ¥ 最终价」
- **个人主页新增面积系数说明卡片**：包含计算规则、举例、显示说明

### 变更
- **移除"请与上级联系"提示**：面积超 10000 不再弹出 toast，改为自动按系数计算
- **更新报价表查询页备注文案**：改为面积系数说明
- **纸张下拉框滚动修复**：添加 `overscroll-behavior:contain` + JS wheel 边界阻止

### 测试
- 更新 `test.html` 测试用例匹配新逻辑（`calcAreaCoefficient` + `matchSpec` 超限不再报错）

---

## [v5.4] - 2026-08-09

### 重大新增
- **报价表组功能**：支持多报价表管理，可切换不同报价表进行价格计算
  - 纸张按报价表隔离（通过 `priceListId` 字段），每个报价表有独立的纸张集合
  - 吊绳/邮费/客户等级为全局共享，不随报价表切换变化
  - 导航栏"1号报价表查询"重命名为"报价表组查询"
  - 报价表组查询页顶部新增报价表切换器（下拉选择 + 删除按钮）
  - 导入 Excel = 新增一个报价表，自动解析"总报价表"行作为报价表名称
  - 导出 Excel = 导出当前报价表的所有 Sheet
  - 可删除当前报价表（不允许删除最后一个）

### 数据架构变更
- `PRICE_LISTS` 数组替代单例 `PRICE_LIST_META`，支持增删报价表
- `CURRENT_PRICE_LIST_ID` 持久化到 localStorage，记录当前选中的报价表
- `PAPER_CONFIG` 每张纸增加 `priceListId` 字段，默认 9 张归 "priceList1"
- `ROPE_CONFIG` / `SHIPPING_CONFIG` 保持全局共享，移除 `priceListId` 字段
- 新增 `addPriceList(name)` / `deletePriceList(id)` 辅助函数
- `EPHEMERAL_KEYS` 为空数组，所有配置均持久化

### 导入导出改造
- `exportPaperExcel()` 仅导出当前报价表的纸张 Sheet
- `parsePaperExcel()` 解析"总报价表"行，返回 `priceListName`
- `importPaperExcel()` 导入时创建新报价表，追加纸张而非替换
- `exportFullData()` / `exportLocalBackup()` 包含 `priceLists` + `currentPriceListId`
- `importFullData()` / `importLocalBackup()` 还原完整报价表组结构
- `createSnapshot()` 快照包含报价表组结构
- 快照恢复同步还原报价表组、纸张、工艺配置

### 修复
- **ID 碰撞修复**：`parsePaperExcel()` 的 `usedIds` 现包含运行时 `PAPER_CONFIG` 所有 ID，防止多次导入生成相同 `paper_import_N`
- **空报价表守卫**：`renderSheets()` / `rebuildPaperUI()` 处理空报价表时不崩溃
- **硬编码修复**：`updateTierOptions()` 改为按当前报价表过滤；`updateDefaultPaperOptions()` 默认项显示当前报价表首张纸

---

## [v5.3] - 2026-08-09

### 修复
- **30+ Sheet 导入 ID 碰撞 Bug**：`parsePaperExcel()` 中不匹配默认简称的纸张 ID 生成逻辑存在碰撞风险
  - 旧逻辑：`id = "paper" + (sheetIndex + 1)`，与默认 `paper1`~`paper9` 冲突
  - 新逻辑：用 `usedIds` Set 去重，不匹配的纸张生成 `paper_import_N` 唯一 ID
  - 修复后 30+ Sheet 导入不再出现纸张价格不可达 / 工艺配置覆盖 / 计算错误
- **工艺 ID 生成**：改用完整 paperId 拼接（`craft_${id}_${n}`），兼容新 ID 格式

### 备份
- 创建 `backup/v5.2.1` 分支和 `v5.2.1-stable` Tag，作为报价表组功能改造前的稳定回退点

---

## [v5.2] - 2026-08-06

### 变更
- **默认客户等级毛利系数修正**：
  - 普通客户：1.3 → **1.2**
  - 优质客户：1.15（不变）
  - 大客户：1.05 → **1.1**
  - 同步更新 `DEFAULT_CUSTOMER_LEVELS`（data.js）和 HTML 静态默认值

### 新增
- **一键恢复全局默认设置**：个人主页新增「全局恢复」卡片
  - 清除浏览器中所有本地配置（纸张 / 工艺 / 吊绳 / 客户等级 / 公司信息 / 个人偏好）
  - 报价历史与本地快照不受影响
  - 操作前弹出确认框，确认后自动刷新页面
- **客户等级删除确认**：删除客户等级时增加确认弹出框
  - 显示被删除等级名称，防止误删
  - 提示可通过「恢复默认」恢复

### 修复
- **浏览器缓存问题**：JS/CSS 引用添加版本号 `?v=5.2`，强制浏览器加载最新文件
  - 修复旧版 `data.js` 被缓存导致全局恢复后系数仍为 1.3/1.15/1.05 的问题
- **v5.2 迁移逻辑**：自动检测 localStorage 中的旧默认系数（1.3/1.15/1.05）并迁移为新默认（1.2/1.15/1.1）
  - 仅迁移与旧默认完全一致的值，用户自定义系数不受影响

---

## [v5.1] - 2026-08-06

### 新增
- **临时毛利系数快速设置**：报价结果区新增临时系数输入框
  - 输入任意系数（如 1.18）后实时显示临时报价卡片（蓝色虚线边框区分）
  - 不影响默认的 3 个客户等级设置，仅当前会话临时显示
  - 空值或无效值时自动隐藏
- **邮费快速修改**：报价结果区新增邮费覆盖输入框
  - 输入修改后的邮费后，单独显示 3 个默认等级的报价卡片（绿色边框区分）
  - 基于「成本合计 - 原邮费 + 新邮费」重新计算，与原始报价区别清晰
  - 配带「清除」按钮，一键恢复
  - 与原始邮费报价区域视觉分离（标签 + 颜色区分）

### 变更
- 报价结果区增加「默认报价（原邮费）」标签，明确区分原始报价和修改邮费后报价

---

## [v5.0] - 2026-08-06

### 重大架构变更
- **多文件拆分**：从 6,285 行单文件 HTML 拆分为多文件结构
  - `css/style.css` — 全部样式（1,481 行）
  - `js/data.js` — 数据配置层 + 存储函数（1,531 行）
  - `js/app.js` — 计算逻辑 + 渲染 + 交互 + 初始化（2,555 行）
  - `index.html` — HTML 骨架 + 脚本引用（826 行）
  - `test.html` — 单元测试骨架（P2）

### P0 安全修复
- **XSS 漏洞修复**：11 处 `innerHTML` 未转义位置全部加上 `escapeHtml()`
  - `renderRopeRadios()` — 吊绳名称/ID 转义
  - `initOptions()` — 地区名称/ID 转义
  - `onCalculate()` — 警告信息转义
  - 报价卡片 — 客户等级名称转义
  - `renderPriceTable()` — 代码列转义
  - `updateDefaultRopeOptions()` / `updateDefaultPaperOptions()` — 下拉选项转义
  - `importLocalBackup()` — 地区重建、文件名转义
  - 纸张下拉选项 — data-paper 属性转义
- **CSP 策略**：`<meta http-equiv="Content-Security-Policy">` 限制 script/style/img/connect 来源
- **SheetJS SRI**：`defer` 加载 + `loadSheetJS()` 按需加载兜底
- **导入数据校验**：新增 `validateImportedData()` 函数
  - 字段类型检查、长度限制（200 字符）、系数范围校验
  - JSON 导入和本地备份导入均接入校验

### P1 性能优化
- **搜索防抖**：报价表搜索 `input` 事件加 200ms `debbounce()`
- **滚动节流**：`scroll` 事件加 100ms `throttle()`，减少下拉关闭频率
- **SheetJS 懒加载**：`loadSheetJS()` 按需加载，不阻塞首屏渲染
  - 8 个 Excel 导入/导出函数全部接入懒加载 + 错误处理
  - `defer` 备用加载确保用户交互前脚本已就绪
- 新增 `debounce()` / `throttle()` 工具函数

### P2 架构改进
- **单元测试骨架**：`test.html` 包含 9 个基础测试用例
  - `parseDecimalPlaces` 边界测试
  - `calcBleedArea` 面积计算测试
  - `matchSpec` 超限错误测试
  - `escapeHtml` XSS 转义测试
  - `hasExactTier` 档位匹配测试
  - `GROUP_META` / `PRICE_LIST_META` 架构元信息测试
  - `PAPER_CONFIG` 默认配置测试
- **"use strict"**：data.js 和 app.js 均启用严格模式
- **文件分离**：CSS/JS/HTML 完全分离，便于维护和版本控制

### 不变项
- 计算逻辑（`calculate()`、`calcBleedArea()`、`matchSpec()` 等）完全不变
- 数据配置（`PAPER_CONFIG`、`CRAFT_CONFIG` 等）完全不变
- Excel 模板导入导出格式完全不变，兼容新旧格式
- 三级架构（小组 → 总报价表 → Sheet 纸张报价表）不变

---

## [v4.3] - 2026-08-06

### 新增
- **三级数据架构明确**：在数据配置区新增架构层级注释和元信息常量
  - `GROUP_META`（小组：1号小组）→ `PRICE_LIST_META`（总报价表：1号报价表）→ Sheet 纸张报价表（PAPER_CONFIG）
  - 明确层级关系：小组 → 总报价表 → Sheet 纸张报价表（含规格 code × 档位价格）
  - 不改动任何现有数据和计算逻辑，仅增加架构概念定义
- **报价表查询页纸张选择器**：新增下拉选择器，可直接选择任意 Sheet 纸张查看报价表
  - Sheet 数量 > 10 时自动显示编号前缀（如"1. 350铜版纸"），方便快速定位
  - 保留原有上一张/下一张按钮，两种导航方式并存
- **计算器纸张下拉编号**：计算器页面的纸张材质下拉在 Sheet > 10 张时也显示编号前缀

### 变更
- **Excel 导出/模板新增架构层级信息**：每个 Sheet 的元数据区新增"所属小组"和"总报价表"两行
  - `paperToSheetRows()` 输出新增 2 行架构元信息
  - `downloadPaperTemplate()` 9 张模板表均新增架构元信息
  - `parsePaperExcel()` 改为按标签匹配读取（不再依赖固定行号），兼容新旧格式
  - 模板 Sheet 命名改为从 meta 中提取"简称"，不再依赖固定索引
- **报价表查询页分页信息优化**：`paperPageInfo` 显示格式从"1 / 9"改为"第 1 / 9 张"
